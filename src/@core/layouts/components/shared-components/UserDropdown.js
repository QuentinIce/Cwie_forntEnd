...
const UserDropdown = () => {
  ...
  useEffect(() => {
    axios
      .post('http://10.21.45.100:3000/api/verify_authen', { //เอาของอั๋นมาแปะ
        token: username,
        tokenRole: role
      })
      .then(data => {
  ...
  useEffect(() => {
    if (user !== undefined && status !== undefined) {
      if (status === 'นักศึกษา') {
        axios.post('http://10.21.45.100:3000/api/ReadStudent', { username: user }).then(data => {
      ...
      if (status === 'อาจารย์') {
        axios.post('http://10.21.45.100:3000/api/ReadTeacher', { username: user }).then(data => {
          if (data.data.length > 0) {
      ...
      if (status === 'เจ้าหน้าที่') {
        axios.post('http://10.21.45.100:3000/api/Readofficer', { username: user }).then(data => {
          if (data.data.length > 0) {
      ...
      if (status === 'สถานประกอบการ') {
        axios.post('http://10.21.45.100:3000/api/Read_Company', { username: user }).then(data => {
          if (data.data.length > 0) {
